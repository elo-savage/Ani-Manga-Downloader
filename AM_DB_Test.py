...
class MangaDownloader:
    ...
    # --- AGGIUNTA NON-BREAKING: progress_cb opzionale ---
    def download_chapter(self, chap_slug: str, chapter_url: str, index: int, progress_cb=None, selection_pos: int | None = None, selection_total: int | None = None):
        ...
        if pdf_path.exists():
            ...
            # Notifica 100% per coerenza in caso di skip
            if progress_cb and selection_pos and selection_total:
                progress_cb(selection_pos, selection_total, chap_slug, 100)
            return
        ...
        try:
            ...
            # Notifica errore come 0% lasciando la gestione all'orchestratore
            if progress_cb and selection_pos and selection_total:
                progress_cb(selection_pos, selection_total, chap_slug, 0)
            return
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for fut in as_completed(futures):
                ...
                try:
                    ...
                    imgs[idx] = path
                    downloaded_pages += 1
                    percent_cap = int(downloaded_pages * 100 / total)
                    # Callback di avanzamento capitolo in tempo reale
                    if progress_cb and selection_pos and selection_total:
                        progress_cb(selection_pos, selection_total, chap_slug, percent_cap)
                    else:
                        logging.info(f"[{index}/{self.total_chapters}] Capitolo {chap_slug} – {percent_cap}% completato")
                except Exception:
                    errors.append(i)
...
def _manga_process_single(url, sel):
    ...
    try:
        ...
        # Selezione valida mantenendo l'ordine scelto dall'utente
        selected = [i for i in sel if 1 <= i <= total_chapters]
        if not selected:
            print(f"[SKIP] Nessun capitolo selezionato valido per '{name}'.")
            return
        selection_total = len(selected)
        completed_chapters = 0

        # Callback di avanzamento: media dei capitoli con contributo del capitolo corrente
        def progress_cb(selection_pos: int, selection_total_local: int, chap_slug: str, percent_cap: int):
            # overall = (capitoli completati + progresso capitolo corrente) / totale selezionati
            overall = int(((completed_chapters + (percent_cap / 100.0)) / selection_total_local) * 100)
            logging.info(f"[{selection_pos}/{selection_total_local}] Capitolo {chap_slug} – {percent_cap}% completato | Download manga – {overall}% completato")

        for pos, i in enumerate(selected, 1):
            slug, chapter_url = chapters[i - 1]
            dl.download_chapter(slug, chapter_url, i, progress_cb=progress_cb, selection_pos=pos, selection_total=selection_total)
            completed_chapters += 1
            # Allineamento finale a 100% dopo chiusura PDF capitolo
            overall_final = int((completed_chapters / selection_total) * 100)
            logging.info(f"[{pos}/{selection_total}] Capitolo {slug} – 100% completato | Download manga – {overall_final}% completato")
    finally:
        dl.close()
