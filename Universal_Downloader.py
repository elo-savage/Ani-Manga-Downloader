...
class MangaDownloader:
    ...
    def get_chapter_links(self) -> list[tuple[str, str]]:
        ...
        for a in anchors:
            ...
            # MangaWorld aggiunge 2 cifre extra agli slug (es. "19104" → "191")
            slug = re.sub(r"\d{2}$", "", slug)
            chapters.append((slug, href))

        numeric = all(re.match(r"^\d+(\.\d+)?$", slug) for slug, _ in chapters)
        if numeric:
            chapters.sort(key=lambda x: float(x[0]) if x[0].replace(".", "", 1).isdigit() else float("inf"))
        else:
            logging.info("Non-numeric chapter IDs, reversing to ascending")
            chapters.reverse()

        logging.info(f"Found {len(chapters)} chapters")
        self.total_chapters = len(chapters)
        if chapters:
            logging.info(f"First: {chapters[0][0]}, Last: {chapters[-1][0]}")
        return chapters

    def _get_first_image_info(self, chapter_url: str):
        page1 = f"{chapter_url}/1"
        resp = self.session.get(page1, timeout=HTTP_TIMEOUT)
        resp.raise_for_status()
        soup = BeautifulSoup(resp.text, "html.parser")

        # Catena di fallback per trovare l'immagine CDN
        patterns = [
            re.compile(r"(?:https?:)?//cdn\.mangaworld\.[^/]+/chapters/", re.IGNORECASE),
            re.compile(r"(?:https?:)?//[^/]*mangaworld\.[^/]+/.*/chapters/", re.IGNORECASE),
            re.compile(r"/chapters/", re.IGNORECASE),
        ]
        img = None
        for pat in patterns:
            img = soup.find("img", src=pat)
    ...
    def _get_total_pages(self, chapter_url: str, soup: BeautifulSoup, img_folder: str, ext: str) -> int:
        # 1. Pattern "X/Y" nel testo
        text = soup.get_text(separator=" ")
        slash = re.findall(r"\b\d+/(\d+)\b", text)
        if slash:
            return max(int(n) for n in slash)

        # 2. Link di navigazione
    ...
    def download_chapter(self, chap_slug: str, chapter_url: str, index: int) -> None:
        ...
        if re.match(r"^\d+(\.\d+)?$", chap_slug):
            pdf_filename = f"Capitolo_{chap_slug}.pdf"
        else:
        ...
        try:
            first_src, folder, ext, soup = self._get_first_image_info(chapter_url)
        except Exception as e:
            if not self.progress_cb: logging.error(f"Error in chapter {chap_slug} first image: {e}")
            return

        total = self._get_total_pages(chapter_url, soup, folder, ext)
        if total < 1:
        ...
        # Directory temporanea unica per tutto il capitolo (verrà eliminata alla fine)
        ...
        try:
            ...
            if errors:
                if not self.progress_cb: logging.error(f"Error downloading pages {errors} in Capitolo {chap_slug}")
                return
...
def manga_rename_decimal_versions(folder: Path) -> None:
    """Rinomina i capitoli interi quando esistono versioni decimali."""
        ...
        try:
            if kind == "anime":
                ...
                try:
                    ...
                    dl.close()
                    manga_rename_decimal_versions(item["output"])
